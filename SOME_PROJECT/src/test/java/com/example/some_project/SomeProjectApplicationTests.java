...
class SomeProjectApplicationTests {
	...
	@Autowired
	SomeProjectApplicationTests(ConfigurableApplicationContext context, DataSourceBean dataSourceBean) {
	...
	void testDataSource() {
		...
		try (Connection connection = dataSource.getConnection()) {
			...
			statement.execute(
					"CREATE TABLE IF NOT EXISTS my_table(id SERIAL PRIMARY KEY," + " name VARCHAR(20) NOT NULL)");
